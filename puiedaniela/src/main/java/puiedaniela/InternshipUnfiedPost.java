...
public class InternshipUnfiedPost {
	...
	public static void main(String[] args) throws Exception {
		...
		float aux = ReturnRON( "https://api.exchangeratesapi.io/" + year + "-" + 1 + "-" + 1 + "?symbols=" + currencyFrom.toUpperCase() + "&base=" + currencyTo.toUpperCase() );
		for(int month = 1; month <= 12; month++) {
			if(month == 2) {
				day = DaysOfFebruary(year);
			}
			else {
				day = MonthDays(month);
			}
			...
			System.out.println("    Calculate for month " + month);
		}
		System.out.println();
	...
	public static float ReturnRON(String url) throws Exception {
		float ronValue;
